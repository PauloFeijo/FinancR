...
export default class CadTransferencia extends Component {
  ...
  handleSubmit(event) {
    ...
    const data = {
      ...
      contaDebitoId: event.target.contaDebito.value,
      contaCreditoId: event.target.contaCredito.value
    }

    let method = 'post'
    let resource = 'transferencia'
    if (this.props.edicao) {
      method = 'put'
      resource = `transferencia\\${this.props.id}`
    }    
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title id="contained-modal-title-vcenter">
              {this.props.edicao ? 'Editar Transferência' : 'Nova Transferência'}
            </Modal.Title>
          </Modal.Header>
          ...
          <Modal.Body>
            <Form onSubmit={this.handleSubmit}>
              ...
              <FieldDropDownDataBind                
                size="12" name="contaDebito" label="Conta Débito" 
                resource="conta" value={this.props.contaDebitoId}  
              />  

              <FieldDropDownDataBind                
                size="12" name="contaCredito" label="Conta Crédito" 
                resource="conta" value={this.props.contaCreditoId}  
              />                                         
